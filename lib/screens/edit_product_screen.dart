...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  // UNLIKE initState that only runs once - didChangeDepencies runs multiple times
  // as dependency of widgets state object changes so we used  a initflag to
  // make sure that widget is not overly rebuilt everytime dependecies change
  @override
  ...
  void _saveForm() {
    ...
    if (productAlreadyHasId) {
      products.updateProduct(_editedProduct).then((value) {
        setState(() => _isLoading = false);
        Navigator.pop(context);
      });
    } else {
      // Made this "then-able" by having addProduct method return a Future / Promise
      // This allows us to have navigation pop only fire once the async http request
      // inside "addProduct" method resolves
      products.addProduct(_editedProduct).catchError((err) {
        // dialog returns a future - so can return showDialog to continue the
        // then chain
        return showDialog(
          context: context,
          builder: (context) => AlertDialog(
            ...
            actions: [
              FlatButton(
                  onPressed: () {
                    // Calling pop will remove the current overlay off the
                    // stack
